...
function viajes_taxonomy() {  
    register_taxonomy(  
        'viajes_categories',  //The name of the taxonomy. Name should be in slug form (must not contain capital letters or spaces). 
        'viajes',        //post type name
        array(  
            'hierarchical' => true,  
            'label' => 'Paises',  //Display name
            'query_var' => true,
            'rewrite' => array(
                'slug' => 'viajes', // This controls the base slug that will display before each term
...
add_action( 'init', 'viajes_taxonomy');
...
function filter_post_type_link($link, $post)
{
    ...
    if ($cats = get_the_terms($post->ID, 'viajes_categories'))
        $link = str_replace('%viajes_categories%', array_pop($cats)->slug, $link);
    return $link;
}
...
function register_themepost() {
    $labels = array(
        'name' => _x( 'viajes', 'my_custom_post','custom' ),
        'singular_name' => _x( 'Viaje', 'my_custom_post', 'custom' ),
        'add_new' => _x( 'Add New', 'my_custom_post', 'custom' ),
        'add_new_item' => _x( 'Add New Viaje', 'my_custom_post', 'custom' ),
        'edit_item' => _x( 'Edit Viaje', 'my_custom_post', 'custom' ),
        'new_item' => _x( 'New Viaje', 'my_custom_post', 'custom' ),
        'view_item' => _x( 'View Viaje', 'my_custom_post', 'custom' ),
        'search_items' => _x( 'Search Viajes', 'my_custom_post', 'custom' ),
        'not_found' => _x( 'No Viajes found', 'my_custom_post', 'custom' ),
        'not_found_in_trash' => _x( 'No Viajes found in Trash', 'my_custom_post', 'custom' ),
        'parent_item_colon' => _x( 'Parent Viaje:', 'my_custom_post', 'custom' ),
        'menu_name' => _x( 'viajes Posts', 'my_custom_post', 'custom' ),
    );

    $args = array(
        'labels' => $labels,
        'hierarchical' => false,
        'description' => 'Custom Viaje Posts',
        'supports' => array( 'title', 'editor', 'excerpt', 'author', 'thumbnail', 'comments', 'revisions', 'post-formats', 'custom-fields' ),
        'taxonomies' => array( 'post_tag','viajes_categories'),
        'show_ui' => true,
        'show_in_menu' => true,
        'menu_position' => 5,
    ...
    register_post_type('have_an_hate_day',
        array(
            'labels' => array(
                'name' => __('Have an hate day'),
                'singular_name' => __('Hate day'),
            	'all_items' => __('All Hate day'),
    ...
    register_post_type('recomendaciones',
        array(
            'labels' => array(
                ...
                'singular_name' => __('Recomendacion'),
            	'all_items' => __('All recomendaciones'),
    ...
    register_post_type('literatura',
        array(
            'labels' => array(
                'name' => __('literatura'),
                'singular_name' => __('Literatura'),
            	'all_items' => __('All Literatura'),
...
/** auto add categori when create some custom type **/
function post_auto_cat( $post_ID ) {

	if(get_post_type($post_ID) == 'have_an_hate_day')
    {
    	$post_type = 'have_an_hate_day';
    	$cat_id = 82; // Your reviews category id (for example: 123)
    }
    else if(get_post_type($post_ID) == 'recomendaciones')
   	{
   		$post_type = 'recomendaciones';
    	$cat_id = 83; // Your reviews category id (for example: 123)
   	} 
   	else if(get_post_type($post_ID) == 'literatura')
   	{
   		$post_type = 'literatura';
    	$cat_id = 84; // Your reviews category id (for example: 123)
    }
